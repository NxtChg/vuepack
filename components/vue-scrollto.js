...
function setDefaults(options){ defaults$$1 = Object.assign({}, defaults$$1, options); }
//_____________________________________________________________________________

var scroller = function()
{
	var element, container, duration, easing, cancelable, abort, onDone, onCancel;
	
	var offset; // offset to be added (subtracted)
	
	var initialX, initialY, targetX, targetY, diffX, diffY; // container
	
	var abortEv; // event that aborted scrolling

	var abortFn = function abortFn(e){ if(cancelable){ abortEv = e; abort = true; } };
	
	var progress, easingFn, timeStart, timeElapsed;
...
function findBinding(el)
{
	for(var i = 0; i < bindings.length; i++)
		if(bindings[i].el === el) return bindings[i];
}
...
function deleteBinding(el)
{
	for(var i = 0; i < bindings.length; i++)
	{
		if(bindings[i].el === el){ bindings.splice(i, 1); return true; }
	}
	return false;
}
...
{
    var binding = findBinding(el); if(binding) return binding;
...
var VueScrollTo$1 = {
    bind:   function bind(el, binding){ getBinding(el).binding = binding; _.on(el, "click", handleClick); },
    unbind: function unbind(el){ deleteBinding(el); _.off(el, "click", handleClick); },
    update: function update(el, binding){ getBinding(el).binding = binding; },
